...
(define (count-leaves bm)
 (+ (count-leaves-branch (left-branch bm)) (count-leaves-branch (right-branch bm)))
  )
(define (count-leaves-branch branch)
  (if (not (pair? (branch-structure branch)))
       1
       (count-leaves (branch-structure branch))
      ))
;; test
